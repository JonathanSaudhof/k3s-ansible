...
| `make nfs-client` | Mount the NFS share on all k3s nodes |
...
setup_nfs_client.yml              mount the NFS share on all k3s nodes (node-level; the
                                   in-cluster StorageClass is installed from ../k3s-configuration)
